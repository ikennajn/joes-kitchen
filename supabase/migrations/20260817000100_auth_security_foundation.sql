...
-- Make the additive migration safe even if an auth user already exists.
...
declare
  ...
  first_user_id uuid;
begin
  if not exists (select 1 from public.household_members) then
    ...
    if target_household_id is not null and first_user_id is not null then
      insert into public.household_members (household_id, user_id, role)
      values (target_household_id, first_user_id, 'owner')
      on conflict do nothing;

      update public.households
      set created_by = first_user_id, updated_at = now()
...
begin
  ...
  foreach table_name in array array[
    ...
    -- Keep this nullable until the final cutover so the legacy anonymous app can
    -- continue inserting rows during the compatibility window.
